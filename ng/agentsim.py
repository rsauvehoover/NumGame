...
class GUI():
    ...
    def __init__(self, init_fn=None, step_fn=None, repick_fn=None,title="Simulation"):
        if GUI.num_instances != 0:
            raise Exception("GUI: can only have one instance of a simulation")
        GUI.num_instances = 1

        self._canvas_x_size = 400
        self._canvas_y_size = 520
        ...
        self._root.bind("<Key-q>", self._do_shutdown_key)
        ...
        def on_range_change(v):
            self._lower_range = int(v)

        self._lower_range = 1
        self._speedscale = Scale(self._frame,
            from_=1, to=12, label='Lower Range', command=on_range_change)
    ...
    def _run(self):
        if self._running:
            if self._step_fn != None:
                ...
                id = self._root.after(150 - self._lower_range, self._run)
...
def init(init_fn=None, step_fn=None,repick_fn=None, title="Simulation"):
    # let us modify the value of the global gui variable
    global gui
    gui = GUI(init_fn=init_fn, step_fn=step_fn, repick_fn = repick_fn, title=title)
